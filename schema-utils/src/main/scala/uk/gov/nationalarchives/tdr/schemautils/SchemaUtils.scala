...
object SchemaUtils {
   ...
   *
   * @param reference
   *  The '$ref' of the Base Schema node.
   *  For example: classpath:/metadata-schema/baseSchema.schema.json#/properties/file_path
   * @return
   ...
   *
   * {
   *  "base_schema_property": {
   *    "alternateKeys": [
   *      {
   *        "alternateHeader": "AlternateName"
   *      }
   *    ]
   *  }
   * }
