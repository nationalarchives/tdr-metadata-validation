...
class ClosurePeriodSpec extends AnyWordSpecLike {
  ...
  "When validating against all schema we see " should {
    ...
    "error(s) if one of the closure periods is a string" in {
      val closedTestFileRow = closedMetadataFileRow(closurePeriod = Some("[100,\"hello\"]"))
      validationErrors(closedTestFileRow) should contain theSameElementsAs List(
        ValidationError(SCHEMA_BASE, "closure_period", "type"), // Must be a number between 1 and 150
        ValidationError(SCHEMA_CLOSURE_CLOSED, "closure_period", "type") // Must be provided for a closed record  //TODO: review double msgs (see below)
      )
    ...
    "error(s) if the value is a boolean" in {
      ...
      validationErrors(closedTestFileRow) should contain theSameElementsAs List(
        ValidationError(SCHEMA_BASE, "closure_period", "type"),
        ValidationError(SCHEMA_CLOSURE_CLOSED, "closure_period", "type") // Must be a number between 1 and 150
      )
    }
